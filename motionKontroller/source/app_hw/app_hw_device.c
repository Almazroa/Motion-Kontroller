 ...
 * Macro Definitions
 ******************************************************************************/
#define R_USR_BTN					   (NC)//(P5_6-staffworeset)(P10_4)
#define R_USR_LED					   (P5_5)
#define R_USR_LED2                     (P7_2)
...
//        cyhal_gpio_write(CYBSP_USER_LED1 , CYBSP_LED_STATE_ON);
        // cyhal_gpio_write(R_USR_LED , CYBSP_LED_STATE_ON);
...
void app_bt_timeout_led_blink(TimerHandle_t timer_handle)
{
    ...
    if (led_on)
    {
    	// cyhal_gpio_write(R_USR_LED, CYBSP_LED_STATE_OFF);
...
void app_bt_interrupt_config(void)
{
    ...
    // result = cyhal_gpio_init(R_USR_BTN,
...
//                             CYBSP_BTN_OFF);
    ...
    // cyhal_gpio_register_callback(R_USR_BTN, &btn_cb_data);
...
//                            true);
    // cyhal_gpio_enable_event(R_USR_BTN,
    //                             CYHAL_GPIO_IRQ_BOTH,
    //                             GPIO_INTERRUPT_PRIORITY,
    //                             true);
...
//    cyhal_gpio_init(R_USR_LED2 , CYHAL_GPIO_DIR_OUTPUT,
//                    CYHAL_GPIO_DRIVE_STRONG, CYBSP_LED_STATE_OFF);
    // cyhal_gpio_init(R_USR_LED , CYHAL_GPIO_DIR_OUTPUT,
                        // CYHAL_GPIO_DRIVE_STRONG, CYBSP_LED_STATE_OFF);
//        cyhal_gpio_init(R_USR_LED2 , CYHAL_GPIO_DIR_OUTPUT,
//                        CYHAL_GPIO_DRIVE_STRONG, CYBSP_LED_STATE_OFF);
...
void button_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        if(CYBSP_BTN_PRESSED == cyhal_gpio_read(R_USR_BTN))
//        if(CYBSP_BTN_PRESSED == cyhal_gpio_read(CYBSP_USER_BTN))
        {
            ...
            // cyhal_gpio_write(R_USR_LED2 , CYBSP_LED_STATE_ON);
        ...
        else if(0 != btn_press_start)
        {
               ...
               (btn_press_duration <= APP_BTN_PRESS_LONG))
            {
                ...
                else
                {
                    ...
                    app_bt_gatt_set_notify_value(R_BTN1);
